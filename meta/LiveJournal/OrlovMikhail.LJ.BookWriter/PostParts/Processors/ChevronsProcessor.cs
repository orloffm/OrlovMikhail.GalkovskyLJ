    ...
    public class ChevronsProcessor : ProcessorBase
    {
        protected internal override void ProcessInternal(List<IPostPart> items)
        {
            ...
            for(int i = -1; i < items.Count; i++)
            {
                ...
                // All text parts, including username items.
                IRendersAsText[] textPartsBetween = EnumerateTextPartsBetween(items, i, nextBreak).ToArray();
        ...
        private void RemoveFormatters(List<IPostPart> items, int before, int after)
        {
        ...
        public static void ReplaceSpecialQuotationWithChevrons(IRendersAsText[] textPartsBetween)
        {
            if(textPartsBetween.Length == 0)
                return;

            RawTextPostPart a = textPartsBetween[0] as RawTextPostPart;
            RawTextPostPart z = textPartsBetween[textPartsBetween.Length - 1] as RawTextPostPart;
            ...
            int totalTextLength = textPartsBetween.OfType<RawTextPostPart>().Sum(q => q.Text.Length);
        ...
        public static int RemoveChevrons(IRendersAsText[] textPartsBetween)
        {
            int chevronCount = 0;
            ...
            {
                RawTextPostPart rtpp = textPartsBetween[p] as RawTextPostPart;
                if(rtpp == null)
                    break;
                ...
                for(int i = 0; i < rtpp.Text.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        if(chevronCount > 0)
                        {
                            ...
                            for(int q = 0; q < p; q++)
                                ((RawTextPostPart)textPartsBetween[q]).Text = String.Empty;
                        }
